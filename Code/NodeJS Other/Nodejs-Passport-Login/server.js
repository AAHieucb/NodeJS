...
app.use(passport.session()) // 1 middleware khác đổi giá trị user là session id(từ cookie của client thành true deserialized user object). Kiểu app.use(passport.authenticate('session')); tức giá trị session phải theo strategy riêng của passport ấy. VD ta dùng cái gì trong cookies thì nó sẽ đi qua passport.session và theo ta xử lý bên trong thì mọi request sẽ dùng được biến req.user
...
*/
// Ta cũng có thể override với query string. VD: dùng như dưới và ta gọi POST vào ?_method=DELETE thì server sẽ chạy biến POST thành DELETE
app.use(methodOverride('_method'))

app.get("/test", (req, res) => {
  // Ta gọi vào get thì làm sao có data mà biết req.user là ai vì ai chả get được. Thật ra k đúng vì mỗi user vào nó đều serialized lưu vào session và deserialized vào req.user nên đã đăng nhập là sẽ có
...
// Post vào trang login để đăng nhập sẽ check authenticate. Tùy vào loại strategy mà nó cần middleware initialize và cả authenticate ở từng url như passport-local
app.post('/login', checkNotAuthenticated, passport.authenticate('local', {
  successRedirect: '/',
  failureRedirect: '/login',
  failureFlash: "Invalid username or password", // failureFlash: true
  successFlash: "Welcome!",
  // Sau khi redirect thì phải thông báo người dùng biết thành công hay thất bại thì cần flash nhưng express 3 k hỗ trợ mà dùng: connect-flash. => dùng middleware flash phải có express-session. Flash message chỉ dùng với web nodejs mới xem được chứ connect database bth thì k
