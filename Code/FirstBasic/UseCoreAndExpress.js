var express = require('express');
var cors = require('cors');
var app = express();

// Dùng cors chỉ ở 1 api cụ thể
app.get('/products/:id', cors(), function (req, res, next) {
    res.json({
        msg: 'This is CORS-enabled for a Single Route'
    })
})

// Cấu hình cors nhiều domain custom
var whitelist = ['', 'https://example2.com']
var corsOptions2 = {
    origin: function (origin, callback) {
        if (whitelist.indexOf(origin) !== -1 || !origin) {
            // Dùng !origin sẽ cho phép các origin undefined truy cập, VD vào web bằng browser bình thường, k gọi từ 1 website khác
            callback(null, true);
        } else {
            callback(new Error('Not allowed by CORS nha'))
        }
    },
    optionsSuccessStatus: 200 // some legacy browsers (IE11, various SmartTVs) choke on 204
}

app.get('/products/:id', cors(corsOptions2), function (req, res, next) {
    res.json({
        msg: 'This is CORS-enabled '
    })
})

app.listen(3000, function () {
    console.log('web server listening on port 3000')
})